...
import { exportYOLO, getProject, type Project } from "../api";
...
export default function ExportPage() {
  ...
  const handleExport = async () => {
    setDownloading(true);
    try {
      const res = await exportYOLO(pid);
      const url = window.URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${project?.name || "project"}_yolo.zip`;
      a.click();
  ...
  return (
    <div className="export-page">
      <div className="export-header">
        <div>
          <h1>Export</h1>
          <p className="subtitle">Download annotations in YOLO format</p>
        </div>
      </div>
      ...
      <div className="panel">
        <div className="row">
          <div>
            <h2 className="panel-title">YOLO dataset (ZIP)</h2>
